...
while li_thickness < 4:
    ...
    ##sketch geometry
    #Bare in mind sheet size is 1000x1000
    height = 342.8
    length = 2900

    ##Sketch (fixed)##
    mdb.models['Model-%d' %(x)].ConstrainedSketch(name='__profile__', sheetSize= 1000)
    ...
    mdb.models['Model-%d' %(x)].materials['Silicon'].Hyperelastic(materialType=ISOTROPIC, table=(
    (2.75E10, 2.507E-11), ), testData=OFF, type=NEO_HOOKE, 
    volumetricResponse=VOLUMETRIC_DATA)
    #lithiated Area
    mdb.models['Model-%d' %(x)].Material(name='Lithiated Silicon')
    mdb.models['Model-%d' %(x)].materials['Lithiated Silicon'].Hyperelastic(materialType=ISOTROPIC, table=(
    (8.135E9, 7.023E-11), ), testData=OFF, type=NEO_HOOKE, 
    volumetricResponse=VOLUMETRIC_DATA)
    mdb.models['Model-%d' %(x)].materials['Lithiated Silicon'].Expansion(table=((2.61e-06, ), ))
    ...
    ##non-linear Step Creation##
    mdb.models['Model-%d' %(x)].StaticStep(initialInc=0.001, maxInc=0.1, name='ThermalBuckling', 
   ...
   edges=mdb.models['Model-%d' %(x)].rootAssembly.instances['Part-1-1'].edges.getByBoundingBox(0,-0.01,0,0,height,0)), u1=ON, u2=ON, ur3=ON, weightingMethod=UNIFORM)
    ...
    mdb.models['Model-%d' %(x)].parts['Part-1'].setElementType(elemTypes=(ElemType(
        elemCode=CPE4H, elemLibrary=STANDARD, secondOrderAccuracy=OFF, 
    ...
    #Create non-linear Buckling Analysis Job
    ...
    mdb.models['Model-%d'%(x)].parts['Part-1'].elements.getByBoundingBox(-0.5,bottom_thickness-seedsize-20,0,length+5,bottom_thickness +5,0), name='bottomli')
    ...
    mdb.models['Model-%d'%(x)].parts['Part-1'].Set(elements=
    mdb.models['Model-%d'%(x)].parts['Part-1'].elements.getByBoundingBox(-0.5,height-top_thickness-1,0,length+5,height-top_thickness+seedsize+20,0), name='topli')
    ...
    with open('C:/temp/S_bottom_SI-job-%d.txt' %(x),'w') as file:
            for m in bottomvalue:
                file.write('%s \n' %m)
                file.write('\n')
    #Logarthmic Strain LE
    # Create a variable that refers to the - the variable from the above frame of step - LStrain 'LE'
    selectvariable = selectframe.fieldOutputs['LE']
    # Create a variable that refers to the element set 'bottomsi' (The set is associated with part instance 'PART-1-1'. USE CAPITAL for elementset name
    selectelement = odb.rootAssembly.instances['PART-1-1'].elementSets['BOTTOMSI']
    ...
    for s in selectregion.values:
        ...
        txtlist.extend(combineelementdata)
        np.savetxt('LE'+'_elements_BottomSi_job-%d.txt'%(x),txtlist,fmt='%s')
    ...
    ##write averages to txt, weird occurances where list bottomvalue[m] not needed, only m when above i needed[] for each indication## 
    with open('C:/temp/LE_bottom_SI-job%d.txt' %(x),'w') as file:
            for m in bottomvalue:
                file.write('%s \n' %m)
                file.write('\n')
    ...
    for s in selectregion.values:
        ...
        combineelementdata= [ 'Element Label = '+str(elementnumber), 'E11 = '+str(elementE11), 'E22 = '+str(elementE22), 'E12 = '+str(elementE12)]
        txtlist.extend(combineelementdata)
        np.savetxt('LE'+'_elements_BottomLi_job-%d.txt'%(x),txtlist,fmt='%s')
    ...
    ##write averages to txt, weird occurances where list bottomvalue[m] not needed, only m when above i needed[] for each indication## 
    with open('C:/temp/LE_bottom_LI-job%d.txt' %(x),'w') as file:
            for m in bottomvalue:
                file.write('%s \n' %m)
                file.write('\n')
    ...
    for s in selectregion.values:
        ...
        combineelementdata= [ 'Element Label = '+str(elementnumber), 'E11 = '+str(elementE11), 'E22 = '+str(elementE22), 'E12 = '+str(elementE12)]
        txtlist.extend(combineelementdata)
        np.savetxt('LE'+'_elements_TopSi_job-%d.txt'%(x),txtlist,fmt='%s')
    ...
    ##write averages to txt, weird occurances where list bottomvalue[m] not needed, only m when above i needed[] for each indication## 
    with open('C:/temp/LE_top_SI-job%d.txt' %(x),'w') as file:
            for m in bottomvalue:
                file.write('%s \n' %m)
                file.write('\n')
    ...
    for s in selectregion.values:
        ...
        combineelementdata= [ 'Element Label = '+str(elementnumber), 'E11 = '+str(elementE11), 'E22 = '+str(elementE22), 'E12 = '+str(elementE12)]
        txtlist.extend(combineelementdata)
        np.savetxt('LE'+'_elements_TopLi_job-%d.txt'%(x),txtlist,fmt='%s')
    ...
    ##write averages to txt, weird occurances where list bottomvalue[m] not needed, only m when above i needed[] for each indication## 
    with open('C:/temp/LE_top_LI-job%d.txt' %(x),'w') as file:
            for m in bottomvalue:
                file.write('%s \n' %m)
